...
function GameServer() {
    ...
    this.config = {
        ...
        serverName: 'Agarmen.com',                  // Server name
        serverWelcome1: 'Welcome to Agarmen.com!',     // First server welcome message
        ...
        serverMinionIgnoreTime: 1, // minion detection disable time on server startup [seconds]
		...
		ejectBombSize: 140.421356237,// Eject size of bombs
		...
		playerMinBombEjectSize: 100, // Minimum size a player cell has to be to eject bomb. (vanilla: mass = val*val/100 = 35 mass)
		...
		enableKBEF: 1               // Enable Key, Bomb, Exlode, Froze systems. (1 to enable, 0 to disable)
		};
...
GameServer.prototype.ejectBomb = function(client) {
    if (!this.canEjectMass(client) || client.frozen || client.cells.length > 1 || client.cells.length < 1)
        return;
        var cell = client.cells[0];
        if (cell._size < this.config.playerMinBombEjectSize)
        ...
        // Create cell and add it to node list
          var ejected = new Entity.Virus(this, null, pos, this.config.ejectBombSize);
